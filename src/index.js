...
class DidomiSDK extends Component {
  ...
  /**
   * Replace the React.element into string which allow us to render HTML
   * @param {*} section, config
   */
  replaceReactElement(section, config) {
    if(config[section] && config[section].content) {
      return {
        ...config[section],
        content: Object.entries(config[section].content).reduce((prev, [key, value]) => ({
          ...prev,
          [key]: Object.entries(value).reduce((prevTrad, [lang, html]) => ({
            ...prevTrad,
            [lang]: renderToString(html)
          }), {})
        }), {})
      }
    } else {
  ...
  /**
   * Parse the config object et return a modified one, valid to be used with our SDK
   * @param {*} config
   */
  parseConfig(config) {
    return {
      ...config,
      notice: this.replaceReactElement('notice', config),
      preferences: this.replaceReactElement('preferences', config)
    }
  }
  ...
  init() {
    ...
    window.didomiConfig = this.parseConfig(this.props.config) || {};
